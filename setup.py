...
class Build(build):
    def run(self):
        if path.isdir("queries"):
            dest = path.join(self.build_lib, "tree_sitter_mdl", "queries")
...
setup(
    ...
    package_data={
        "tree_sitter_mdl": ["*.pyi", "py.typed"],
        "tree_sitter_mdl.queries": ["*.scm"],
    },
    ext_package="tree_sitter_mdl",
    ext_modules=[
        Extension(
            name="_binding",
            sources=[
                "bindings/python/tree_sitter_mdl/binding.c",
                "src/parser.c",
            ],
            define_macros=[
